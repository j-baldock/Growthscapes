...
Daily survival probability follows a logistic sigmoid in weight, rising from a floor (`minprob`) for the smallest fish to an asymptote (`maxprob`) for large fish. The inflection weight `w0` sets the size at which survival is halfway between the two extremes. Elliott (1993, Fisheries Research) estimates 6.4% daily mortality during the critical period for BNT (`minprob = 0.936`); `w0 = 7`g corresponds to approximately 60 days post-hatch in the cold habitat (based on age-0 daily weight trajectory plots in the Sensitivity Analysis). Mortality is therefore most intense and most sustained for fish below `w0`, which creates growth-mediated density dependence: high density → reduced food → slower growth → more time below `w0` → higher cumulative mortality.
...
Plot daily survival probability and 60-day cumulative survival by fish weight. The 60-day cumulative assumes constant weight (i.e., no growth) and therefore represents the maximum mortality a fish could experience at each size — in practice cumulative survival will be higher as fish grow through the danger zone.

```{r}
#| purl: false

myweights <- seq(from = 0, to = 20, by = 0.05)

surv.list  <- fncSurviveSize(myweights)
df_size <- tibble(
  weight = myweights,
  daily  = surv.list[[1]],
  cum60  = daily^60
...
p1 <- ggplot(df_size, aes(x = weight, y = daily)) +
  geom_line() +
  geom_vline(xintercept = 5, linetype = "dotted", color = "gray50") +
  annotate("text", x = 5.3, y = 0.935, label = "w0 = 5g", hjust = 0, size = 3, color = "gray40") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) +
  labs(x = "Fish weight (g)", y = "Daily survival probability",
       title = "Daily survival probability by weight") +
  theme_bw()

p2 <- ggplot(df_size, aes(x = weight, y = cum60)) +
  geom_line() +
  geom_vline(xintercept = 5, linetype = "dotted", color = "gray50") +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  labs(x = "Fish weight (g)", y = "60-day cumulative survival",
       title = "60-day cumulative survival by weight") +
  theme_bw()
...
expand_grid(
  asymp = seq(from = 0.9986, to = 0.9998, by = 0.0002),
  days = seq(from = 0, to = 7500, by = 100)
) %>% 
...
`fncSurviveConsumption()` applies density-dependent mortality during the critical period (first
`crit_period_days` days post-hatch), motivated by Elliott (1989): newly-hatched fry with minimal
energy reserves face sharply elevated mortality when daily consumption falls below the level
needed to sustain growth. The function returns a **daily** survival probability, parameterised so
that a fish sustaining `crit_pcmax_lo` throughout the entire critical period has approximately 1%
period-level survival, while one sustaining `crit_pcmax_hi` has approximately 99% survival. Fish
outside the critical period (`age_days > crit_period_days`) or with `NA` pcmax receive a daily
probability of 1.0 (no additional mortality). Designed for use with the hyperbolic
density-dependent ration; `pcmax_adjusted_dd` is the direct input.

```{r fncSurviveConsumption}
...
`fncSurviveAge()` applies a senescent mortality penalty that increases smoothly with age, using a logistic sigmoid centered on an inflection point `x0`. The logistic form allows mortality to rise gradually over many years, consistent with the diffuse nature of senescence in long-lived fish. The function is structurally symmetric with the logistic forms used for temperature and starvation survival.

$$
p_{\text{age}} = p_{\min} + \frac{1 - p_{\min}}{1 + e^{k\,(\text{age} - x_0)}}
$$

For young fish (age $\ll x_0$) the function returns values close to 1.0; for very old fish (age $\gg x_0$) it approaches $p_{\min}$ asymptotically. The steepest decline occurs at the inflection point `x0`, where daily survival equals $(1 + p_{\min})\,/\,2$.

Parameters: default values are tuned so that senescence begins to add meaningful mortality around age 8–9, annual survival from aging alone crosses 50% near age 12, and fish rarely persist past age 17.

-   `age`: numeric vector of fish ages **in years**. In the IBM loop, pass `age_days / 365`.
-   `x0`: inflection point — age (years) at which the senescent decline is steepest; default 14. Onset is gradual roughly 4–6 years before `x0`.
-   `k`: steepness of the logistic decline; larger values produce a sharper drop. Default 0.7.
...
- Lorenzen, K., Camp, E. V., & Garlock, T. M. (2022). Natural mortality and body size in fish populations. Fisheries Research, 252, 106327.
- Dureuil, M., & Froese, R. (2021). A natural constant predicts survival to maximum age. Communications Biology, 4(1), 641.
...
data.frame(
  ...
  # geom_vline(xintercept = c(9), linetype = "dashed", alpha = 0.4) +
  geom_line(linewidth = 0.9) +
  facet_wrap(~ metric, scales = "free_y") +
  theme_bw() + theme(panel.grid = element_blank()) +
  xlab("Age (years)") + ylab("Survival probability") +
  labs(title = "Senescence-based daily survival (x0 = 14, k = 0.7, p_min = 0.99)")
```
...
expand_grid(
  ...
  mutate(
    p        = fncMaturityDate(doy, peak_doy = 121, duration = duration),
...
p_heat <- expand_grid(
  ...
  mutate(
    p_date  = fncMaturityDate(doy, peak_doy = 120, duration = 15),
