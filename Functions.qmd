...
This is the same as above but instead of passing the function a baseline ration, pass it the rations we computed based on temperature-based P_C~max~ and size-based C~max~. *Subtract 1 from density so that density-dependence only kicks in when N \> 1*
