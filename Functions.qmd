...
*Option 3: Allometric metabolic scaling (Fullerton-inspired)*
...
`fncMoveCost_power` seems like the most biologically realistic form. Repeat the simulation but with different parameter values
```{r}
w_seq <- seq(0.1, 150, by = 0.1)
...
```
`fncMoveCost_power` with `c = 0.025` and `a = 1` seems like a reasonable starting place
...
Fish experience mortality based on their size (cumulative experience) and instantaneous growth rate (measure of current conditions). Smaller fish with lower growth rates more likely to die (sampled probabilistically at each time step).
