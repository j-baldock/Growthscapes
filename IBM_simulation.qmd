...
* Movement cost
  + Power function a (decay exponent) and c (scaling constant)
...
movecost_c        <- 0.025 # movement cost power function: `c` = scaling constant; `
movecost_a       <- 1      # movement cost power function `a` = decay exponent (higher = steeper drop-off)
...
for (d in 1:n_days) {
  ...
  if (length(mover_rows) > 0) {
    ...
    # If relevant, penalize the alternative patch by size-dependent energetic cost of movement
    move_cost_vec <- fncMoveCost_power(fish_pop$weight[mover_rows], a = movecost_a, c = movecost_c)
