...
**Purpose:** Define `plot_mechanistic()`, `plot_population()`, and `plot_patchvalue()`, the wrappers to return standardized plots sets of the IBM results. 


## `plot_mechanistic()`

Produce standard mechanistic diagnostic plots from IBM output. Covers bioenergetics, temperature and growth experienced, size trajectories, maximum age, survival decomposition, movement, and reproduction.

**Arguments:**

* `ibm_long` — long-format individual records from `run_simulation()`
* `habitat_df` — habitat time series from `build_habitat()`
* `sim_params` — simulation parameter list (used for survival anchor values and `start_wt`)
* `spawn_log`, `fish_registry` — from `run_simulation()`
* `exp_start_date` — optional `Date`; draws a vertical dashed line separating burn-in from experiment
...
Produce standard population-level outcome plots from IBM output. Covers daily and annual abundance/biomass, age structure, weight-at-age, body-size distributions, survival, lambda, and cohort survivorship.
...
Produce standard habitat-value plots for optimal movers: occupancy, total biomass, per-area biomass, growth rate, and daily survival — each shown as annual time series, seasonal time series, and equivalence curves.
...
#| eval: false
#| purl: false
file.remove("PlotResults.R")
quarto::qmd_to_r_script("PlotResults.qmd")
```