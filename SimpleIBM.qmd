...
for(fish.mass in c(1, 10, 100, 250, 500, 1100)){
  plot(waterTemps[,"WT"], wt.growth[, 16, fish.mass], type='l', ylab = ylb, xlab = "Water temperature (C)", 
  ...
  for(i in seq(2, length(rations)-1, by = 2)){ lines(waterTemps[,"WT"], wt.growth[,i, fish.mass], col = "gray70")}
  abline(h = 0, lty = 2)
}
```

Plot optimal growth temperature and maximum growth rate by fish size
```{r}
dim(wt.growth)[3]
opttemptib <- tibble(weight = weights, opttemp = NA, growth_ggd = NA)

for (i in 1:dim(wt.growth)[3]) {
  opttemptib$opttemp[i] <- waterTemps[which.max(wt.growth[,16,i]),2]
  opttemptib$growth_ggd[i] <- wt.growth[which.max(wt.growth[,16,i]),16,i]
}

opttemptib %>% ggplot() +
  geom_line(aes(x = weight, y = opttemp)) +
  theme_bw() + xlab("Fish weight (g)") + ylab("Optimal temperature for growth (C)")

opttemptib %>% ggplot() +
  geom_line(aes(x = weight, y = growth_ggd)) +
...
growthregime_df %>% ggplot() +
  ...
  theme_bw() +
  xlab("Date") +
  ylab("Cumulative growth potential (g/g/day)")
```
...
Unlike in Armstrong et al., here we simulate the growth regime where growth potential is an additional function of fish mass to generate more realistic size trajectories. This will also serve as the basis of the IBM. 

*something is not right here...fish are growing wayyyy too fast. Check out lines 392-420 of Fullerton's growth_regime_IBM_v1.0.1.R script..."grow fish" section*
...
for (i in 1:dim(growthregime_df_2)[1]) {
  if (i == 1) {
    grpot_warm <- wt.growth[which(waterTemps[,2] == growthregime_df_2$temp_warm[i]),16,startweight]
    grpot_cold <- wt.growth[which(waterTemps[,2] == growthregime_df_2$temp_cold[i]),16,startweight]
    ...
    grpot_warm <- wt.growth[which(waterTemps[,2] == growthregime_df_2$temp_warm[i]),16,round(growthregime_df_2$weight_warm[i-1], digits = 0)]
    grpot_cold <- wt.growth[which(waterTemps[,2] == growthregime_df_2$temp_cold[i]),16,round(growthregime_df_2$weight_cold[i-1], digits = 0)]
