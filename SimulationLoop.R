...
run_simulation <- function(habitat_df, params = list(), wt_growth) {
  ...
  for (d in seq_len(n_days)) {
    ...
    # Applied only to fish within their first crit_period_days days post-hatch.
    # Uses pcmax_adjusted_dd (density-adjusted daily consumption rate) computed in Step 3.
    # Fish added this day (new offspring) have NA pcmax_adjusted_dd and receive p_crit = 1.0.
    ...
    prb.srv   <- pmin(p_sg * p_temp * p_starv * p_crit, 1)
