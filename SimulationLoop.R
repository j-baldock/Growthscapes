  ...
  for (d in seq_len(n_days)) {
    ...
    if (length(mover_rows) > 0) {
      ...
      if (sense_environment == "abiotic_only") {
        ...
        # rowSums gives the dominance-weighted count of existing residents as perceived by the focal
        # fish (no +1 for self, since we want competitors only, consistent with eff_dens_current_vec).
        eff_dens_if_join_warm <- rowSums(
          outer(mover_weights_vec, warm_weights,
                FUN = function(mi, mj) ifelse(mj >= mi, 1, (mj / mi)^dominance_beta))
        ) / A_warm

        eff_dens_if_join_cold <- rowSums(
          outer(mover_weights_vec, cold_weights,
                FUN = function(mi, mj) ifelse(mj >= mi, 1, (mj / mi)^dominance_beta))
        ) / A_cold
    ...
    # after fish have moved. eff_density excludes self (+1 from fncEffDensity removed) and is
    # divided by patch area, giving per-area competitor density. Set dominance_beta = 0 for
    # pure scramble; K is calibrated in the same fish-per-unit-area units.
    fish_pop <- fish_pop |>
      group_by(patch) |>
      mutate(
        A_patch     = if_else(first(patch) == "warm", A_warm, A_cold),
    ...
    if (d > 1) {
      ...
      current_year <- year(habitat_df$date[d])
    ...
    # Weight and size dependent survival — growth effect is rescaled to a specific range
    p_sg      <- fncSurviveSize(fish_pop$weight, maxprob = s_max_vec)[[1]]
